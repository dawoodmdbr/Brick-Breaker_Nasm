...
rage_quit_str: db 'ESC TO RAGE QUIT'
...
main_game:
	...
	nextcmp4:
		...
		jmp exit
	nextcmp5:
	    cmp al, 0xb9        ; space bar released
	    jne check_e_key
	    jmp exit

	check_e_key:
	    cmp al, 0x12        ; E key pressed (not released)
	    jne check_q_key
	...
	check_q_key:
	    cmp al, 0x10        ; Q key pressed
	    jne check_r_key
	    cmp byte[cs:end_game], 1  ; Only allow quit on end screen
	    jne exit
	    mov byte[cs:quit], 1
	    jmp exit

	check_r_key:
	    cmp al, 0x13        ; R key pressed
	    jne nomatch
	    cmp byte[cs:end_game], 1  ; Only allow restart on end screen
	    jne exit
...
do_restart:
    ...
    ; [Add code to restore brick arrays to initial values]
